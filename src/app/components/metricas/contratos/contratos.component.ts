...
export class ContratosComponent {
  ...
  distribucionSexo: { nombre: string; total_paquetes: number }[] = [];
  distribucionRegistros: { nombre: string; total_documentos: number }[] = [];
  ...
  fetchDistribucionSexo():void{
    this.serv.getData('metrics/paquetes-contratos').subscribe({
      next: (data: any) => {
  ...
  fetchRegistrosDiarios():void{
    this.serv.getData('metrics/documentos-contratos').subscribe({
      next: (data: any) => {
        console.log('Datos recibidos:', data);
  ...
  createChartRegistros(): void{
    ...
    if (ctx) {
      new Chart(ctx, {
        type: 'pie',
        data: {
          labels: this.distribucionRegistros.map(d => d.nombre),
          datasets: [{
            label: 'Registros',
            data: this.distribucionRegistros.map(d => d.total_documentos),
            backgroundColor: [
    ...
    if (ctx) {
      new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              data: this.distribucionSexo.map(d => d.total_paquetes),
              backgroundColor: [
                '#FF99A5', '#FFC966', '#FFEE99', '#6FD7E3', '#BFA1E8',
                '#FFAA75', '#94E8A6', '#6EC7FF', '#BCAAA4', '#FF9DAA',
                '#E6E06C', '#D69CF2', '#A8B7F3', '#FFB5D1', '#FFDB7F',
